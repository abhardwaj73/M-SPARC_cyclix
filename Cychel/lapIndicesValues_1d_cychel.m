function S = lapIndicesValues_1d_cychel(S)
...
% Diagonal matrix for 1/r^2
R = (S.xin + (0:Nx-1)*dx);
if S.cell_typ == 3
    r2inv = 1./(R.*R);
elseif (S.cell_typ == 4 || S.cell_typ == 5)
    r2inv = (1/(S.twist*S.twist))* (1./(R.*R));
end
S.R2inv = spdiags(r2inv',0,Nx,Nx);
